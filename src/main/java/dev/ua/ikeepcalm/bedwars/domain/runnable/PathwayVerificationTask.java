...
public class PathwayVerificationTask extends BukkitRunnable {
    ...
    private void verifyPlayerPathway(Player player, Arena arena) {
        ...
        if (!circleOfImaginationAPI.isVirtualBeyonder(player)) {
            log.info("Player {} has magic data but no active sandbox loadout, reinitializing", player.getName());
            plugin.getArenaPathwayManager().initializePlayerMagic(player, arena, team);
            return;
        }
        ...
        if (beyonderData == null || beyonderData.pathways().isEmpty()
            || !beyonderData.pathways().getFirst().name().equals(teamPathway)) {
            // Re-entering swaps the loadout in place, so there is nothing to tear down first.
            log.info("Player {} has an incorrect sandbox pathway, reinitializing", player.getName());
            plugin.getArenaPathwayManager().initializePlayerMagic(player, arena, team);
        }
    }
}