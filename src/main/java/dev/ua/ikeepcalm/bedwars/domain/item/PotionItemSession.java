...
public class PotionItemSession extends SpecialItemUseSession {
    ...
    public void run() {
        ...
        // Re-entering swaps the sandbox loadout in place; the player's real Beyonder stays stashed.
        if (!circleOfImaginationAPI.enterVirtualBeyonder(player, pathwayData.getFirst().name(), sequence)) {
            return;
        }

        data.setCurrentSequence(sequence);
        data.setStoredActing(0);
